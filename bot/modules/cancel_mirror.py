    ...
    if len(args) == 1:
        if mirror_message is None or mirror_message.message_id not in keys:
            if BotCommands.MirrorCommand in mirror_message.text or \
                ...
                msg = "Mirror telah dibatalkan"
                sendMessage(msg, context.bot, update)
                return
            else:
                msg = "Tolong balas ke pesan /mirror yang digunakan untuk memulai download atau /batal gid untuk membatalkan!"
                sendMessage(msg, context.bot, update)
                return
    if dl.status() == "Uploading":
        sendMessage("Upload sedang berlangsung.", context.bot, update)
        return
    elif dl.status() == "Archiving":
        sendMessage("Sedang mengarsipkan.", context.bot, update)
        return
    else:
    ...
    with download_dict_lock:
        ...
        for dlDetails in list(download_dict.values()):
            if dlDetails.status() == MirrorStatus.STATUS_DOWNLOADING \
                ...
                count += 1
    delete_all_messages()
    sendMessage(f'Membatalkan {count} download!', context.bot, update)
